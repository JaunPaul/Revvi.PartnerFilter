...
const filtersForm = document.querySelector(
  'form[fs-list-element="filters"], .product-filtering-wrap',
);
const searchInput = filtersForm?.querySelector('input[type="text"]');
const API_BASE_URL =
  window.__REVVI_PARTNER_FILTER_API_BASE__?.replace(/\/$/, "") ??
...
function renderPagination(
  ...
  host.style.display =
    pagination.hasPreviousPage || pagination.hasNextPage ? "" : "none";
...
async function loadPage(
  page,
  { pushState = true, searchParams = buildSearchParams() } = {},
) {
  ...
  try {
    ...
    if (statusHost) {
      statusHost.innerHTML =
        '<div class="partner-page__error">Unable to load partners right now.</div>';
    }
  }
}
...
window.addEventListener("popstate", () => {
  ...
  loadPage(parsePage(window.location.search), {
    pushState: false,
    searchParams,
  });
});
